...
{
  kDebug() << "FX_SetupImpl::slotRemoveAll()";
...
{
    kDebug() << "FX_SetupImpl::slotReload()";
    
    Init(pluginlist);
}


void FX_SetupImpl::slotConfigureEffect(Q3ListBoxItem *item)
{
...
void FX_SetupImpl::slotSaveWasClicked()
{
  ...
  slist.clear();
  for(uint i=0; i<listBox_Active->count(); i++){
    slist.append( listBox_Active->text( (int)i) );
  }
  // Write StringList
